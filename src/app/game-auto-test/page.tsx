...
export default function GamePage() {
  ...
  // Ref para armazenar o estado mais recente do jogo
  ...
  const [autoTestSpeed, setAutoTestSpeed] = useState(1000);
  ...
  // Atualizar ref sempre que gameState mudar
  useEffect(() => {
    gameStateRef.current = gameState;
  }, [gameState]);

  // Função wrapper para salvar o estado e atualizar o ref
  const saveGameStateAndUpdateRef = (newState: typeof gameState) => {
    if (!newState) return;
  ...
  // Resetar jogo para modo de teste
  ...
  useEffect(() => {
    ...
    const autoPlayInterval = setInterval(async () => {
      // Obter o estado mais recente do ref
      const currentState = gameStateRef.current;
      if (!currentState) return;

      const currentPlayer =
        currentState.players[currentState.currentTurnPlayerIndex];
      const enemies = currentState.players.filter(
      ...
      } else if (decision.action === "attack_boss" && currentState.activeBoss) {
        actionName = `ATACAR BOSS`;
        actionResult = `👹 ${
          currentState.activeBoss.name
        } | HP: ${currentState.activeBoss.currentHp.toLocaleString()}/${currentState.activeBoss.maxHp.toLocaleString()}`;
      ...
      try {
        // Garantir que estamos usando o estado mais recente do ref
        const latestState = gameStateRef.current;
        if (!latestState) return;

        console.log("🔍 [DEBUG] Estado antes da ação:");
        console.log(
          `   Atacante: ${digimonToAct.name} - HP: ${digimonToAct.currentHp}`
        );
        if (decision.target) {
          console.log(
            `   Defensor: ${decision.target.name} - HP: ${decision.target.currentHp}`
          );
        }

        let updatedState = { ...latestState };

        // AUTO-TEST: se o Digimon atual já pode evoluir, evoluir imediatamente antes de outras ações
        {
          const allDigimonsNow = latestState.players.flatMap((p) => p.digimons);
          const currentNow = allDigimonsNow.find(
            (d) => d.id === digimonToAct.id
          );
          const hasEvos =
            currentNow &&
            currentNow.evolution &&
            currentNow.evolution.length > 0;
          if (
            currentNow &&
            currentNow.currentHp > 0 &&
            currentNow.canEvolve &&
            hasEvos
          ) {
            ...
            if (latestState.activeBoss)
              existingIds.add(latestState.activeBoss.id);

            // Escolher primeira opção que não existe em campo
            const chosenId = (currentNow.evolution || []).find(
              (evId) => !existingIds.has(evId)
            );

            if (!chosenId) {
              console.log(
                `⚠️ [EVOLVE] Nenhuma forma disponível sem duplicar em campo. Evolução adiada para ${currentNow.name}.`
              );
              // Não evoluir agora; segue o fluxo normal
            } else {
              const next = latestState.players
                .flatMap((p) => p.digimons)
                .concat(
                  latestState.activeBoss
                    ? ([latestState.activeBoss] as any)
                    : []
                )
                .find((d: any) => d.id === chosenId) as any;
              if (!next) {
                console.log(
                  `⚠️ [EVOLVE] Próxima forma não encontrada no catálogo. Evolução adiada para ${currentNow.name}.`
                );
              } else {
                // Sem DP fixo: manter HP atual
                const evolvedHp = Math.max(1, currentNow.currentHp);
                    ...
                    digimons: player.digimons.map((d) => {
                      if (d.id === currentNow.id) {
                        return {
                          ...
                          evolutionProgress: 0,
                          canEvolve: false,
                          lastEvolutionTurn: latestState.turnCount,
                          temporaryEvolution: undefined,
                ...
                console.log(
                  `✨ [EVOLVE] ${currentNow.name} evoluiu para ${next.name} (HP ${evolvedHp})`
                );

                // Salvar e avançar turno
                saveGameStateAndUpdateRef(updatedState);
                setTimeout(() => {
                  handleNextTurn();
                }, 300);
                return;
        ...
        if (decision.action === "attack" && decision.target) {
          // Buscar os digimons atualizados do estado atual
          const allDigimons = latestState.players.flatMap((p) => p.digimons);
          const currentAttacker = allDigimons.find(
            (d) => d.id === digimonToAct.id
          );
          const targetId = decision.target.id;
          const currentDefender = allDigimons.find((d) => d.id === targetId);

          if (!currentAttacker || !currentDefender) {
            console.error(
              "❌ [AUTO-TEST] Digimon não encontrado no estado atual"
            );
            return;
          }

          console.log("🔍 [DEBUG] Digimons encontrados no latestState:");
          console.log(
            `   Atacante: ${currentAttacker.name} - HP: ${currentAttacker.currentHp}/${currentAttacker.dp}`
          );
          console.log(
            `   Defensor: ${currentDefender.name} - HP: ${currentDefender.currentHp}/${currentDefender.dp}`
          );

          // Executar ataque PvP diretamente no estado
          const attackerModifier = getStatusDamageModifier(currentAttacker);
          const defenderModifier = getStatusDamageModifier(currentDefender);
          ...
          console.log(
            `⚔️ [DEBUG] Resultado da batalha: ${result.attackerDamage} / ${result.defenderDamage}`
          );
          ...
          console.log(`🔍 [DEBUG] Resultado do dano aplicado:`);
          console.log(
            `   Atacante: HP ${attackerResult.newHp}, XP ${attackerResult.newProgress}%`
          );
          console.log(
            `   Defensor: HP ${defenderResult.newHp}, XP ${defenderResult.newProgress}%`
          );
          ...
          console.log(
            `⚔️ [AUTO-TEST] Ataque executado: ${currentAttacker.name} → ${currentDefender.name}`
          );
          console.log(
            `   Dano: ${result.attackerDamage} / ${result.defenderDamage}`
          );
          console.log(
            `   HP: ${currentAttacker.name} ${attackerResult.newHp}/${currentAttacker.dp}, ${currentDefender.name} ${defenderResult.newHp}/${currentDefender.dp}`
          );
        } else if (decision.action === "evolve") {
          // Evoluir automaticamente (auto-test): escolher a primeira opção disponível
          const allDigimons = latestState.players.flatMap((p) => p.digimons);
          const current = allDigimons.find((d) => d.id === digimonToAct.id);

          if (!current) {
            console.error("❌ [AUTO-TEST] Digimon não encontrado para evoluir");
            return;
          }

          const options = current.evolution || [];
          if (options.length === 0) {
            console.log("⚠️ [EVOLVE] Nenhuma evolução disponível");
            return;
          }
          // Evitar duplicatas em campo: coletar IDs já presentes (inclui boss)
          const existingIds = new Set<number>();
          ...
          // Escolher primeira opção que não existe em campo
          const chosenId2 = options.find((evId) => !existingIds.has(evId));
          const chosen = chosenId2
            ? allDigimons.find((d) => d.id === chosenId2)
            : undefined;
          if (!chosen) {
            console.log(
              `⚠️ [EVOLVE] Nenhuma forma disponível sem duplicar em campo. Evolução adiada para ${current.name}.`
            );
          } else {
            const evolvedHp = Math.max(1, current.currentHp);
            ...
            console.log(
              `✨ [EVOLVE] ${current.name} evoluiu para ${chosen.name} (HP ${evolvedHp})`
            );
          }
        } else if (
          ...
          // Buscar o digimon atualizado do estado atual
          const allDigimons = latestState.players.flatMap((p) => p.digimons);
          const currentAttacker = allDigimons.find(
            (d) => d.id === digimonToAct.id
          );
          ...
          if (!currentAttacker) {
            console.error(
              "❌ [AUTO-TEST] Digimon atacante não encontrado no estado atual"
            );
            return;
          }
          ...
          const updatedBoss = { ...latestState.activeBoss };
          updatedBoss.currentHp = Math.max(
            0,
            updatedBoss.currentHp - result.attackerDamage
          );

          // Rastrear aggro
          const lastAttackers = updatedBoss.lastAttackers || [];
          const existingIndex = lastAttackers.findIndex(
            (a) => a.digimonId === currentAttacker.id
          );
          if (existingIndex >= 0) {
            lastAttackers[existingIndex].damage += result.attackerDamage;
          } else {
            lastAttackers.push({
              digimonId: currentAttacker.id,
          ...
          console.log(
            `👹 [AUTO-TEST] Ataque ao boss: ${currentAttacker.name} → ${latestState.activeBoss.name}`
          );
          console.log(`   Dano: ${result.attackerDamage}`);
          console.log(
            `   Boss HP: ${updatedBoss.currentHp}/${updatedBoss.maxHp}`
          );
        } else if (decision.action === "explore") {
          // Buscar o digimon atualizado do estado atual
          const allDigimons = latestState.players.flatMap((p) => p.digimons);
          ...
          updatedState = {
            ...
            players: latestState.players.map((player) => ({
              ...player,
              digimons: player.digimons.map((d) => {
                if (d.id === currentDigimon.id) {
                  const newProgress = Math.min(
                    100,
                    (d.evolutionProgress || 0) + 10
                  );
                  return {
                    ...d,
        ...
        // Salvar estado atualizado e atualizar ref
        saveGameStateAndUpdateRef(updatedState);

        console.log("💾 [DEBUG] Estado salvo. Verificando ref atualizado...");
        setTimeout(() => {
          const refState = gameStateRef.current;
          if (refState) {
            const refAttacker = refState.players
              .flatMap((p) => p.digimons)
              .find((d) => d.id === digimonToAct.id);
            const refDefender =
              refState.players
                .flatMap((p) => p.digimons)
                .find((d) => d.id === (decision.target?.id ?? -1)) || null;
            console.log("🔍 [DEBUG] Estado do ref após salvar:");
            if (refAttacker) {
              console.log(
                `   Atacante: ${refAttacker.name} - HP: ${refAttacker.currentHp}`
              );
            }
            if (refDefender) {
              console.log(
                `   Defensor: ${refDefender.name} - HP: ${refDefender.currentHp}`
              );
            }
          }
        }, 100);
  ...
  // Verificar condição de término (auto-test: sem telas de vitória/derrota)
  useEffect(() => {
    if (!gameState || showAttackDialog) return;

    const playersWithAliveDigimons = gameState.players.filter((player) =>
      player.digimons.some((digimon) => digimon.currentHp > 0)
    );

    // Se ninguém tem Digimon vivo: parar auto-test e não mostrar telas
    if (playersWithAliveDigimons.length === 0) {
      if (isAutoTestActive) {
        setIsAutoTestActive(false);
        enqueueSnackbar("🧪 Teste encerrado: nenhum Digimon vivo.", {
          variant: "info",
        });
        addTestLog(
          "SISTEMA",
      ...
      if (!hasBossAlive) {
        if (isAutoTestActive) {
          ...
          addTestLog(
            ...
            "Fim do Teste",
            "VITÓRIA",
            `${winnerPlayer.name} venceu com ${aliveDigimons.length} Digimon(s) vivo(s)`
          );
        }
      } else {
        console.log(
  ...
  }, [gameState, showAttackDialog, isAutoTestActive, enqueueSnackbar]);
  ...
  const handleNextTurn = () => {
    ...
    let updatedBoss = state.activeBoss;

    if (isLastPlayer && state.activeBoss && !state.activeBoss.isDefeated) {
      console.log("🌍 [BOSS] Executando Turno do Mundo (fim da rodada)...");

      // Atualizar aggro do boss antes do ataque
      updatedBoss = { ...state.activeBoss };
      ...
      enqueueSnackbar(
        `🌍 TURNO DO MUNDO! ${state.activeBoss.name} atacou ${
    ...
    // Salvar usando o wrapper que também atualiza o ref
    saveGameStateAndUpdateRef(updatedState);
  };
  ...
  const applyDamageToDigimon = (
    ...
    if (!digimon.canEvolve && newProgress > 0 && newHp > 0 && hasEvolutions) {
      ...
      if (turnosSinceEvolution >= evolutionCooldown) {
        // Auto-test: tornar evolução determinística para validar fluxo
        if (isAutoTestActive && newProgress >= 40) {
          evolutionUnlocked = true;
          enqueueSnackbar(
            `🌟 EVOLUÇÃO LIBERADA! ${capitalize(
              digimon.name
            )} pode evoluir! (AUTO-TEST, XP ${newProgress.toFixed(0)}%)`,
            { variant: "success" }
          );
        } else {
        ...
        <div className="container mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 w-full sm:w-auto">
              ...
              {/* Botão Auto-Test */}
              <button
                onClick={() => setIsAutoTestActive(!isAutoTestActive)}
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {isAutoTestActive ? "Auto-Test Ativo" : "Auto-Test"}
                </span>
              </button>
              ...
              <button
                ...
                className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 text-white text-sm sm:text-base font-semibold rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-1 sm:gap-2"
              >
                <span>📋</span>
              ...
              <button
                ...
                className="px-3 sm:px-4 py-1.5 sm:py-2 bg-orange-600 text-white text-sm sm:text-base font-semibold rounded-lg hover:bg-orange-700 transition-colors flex items-center gap-1 sm:gap-2"
              >
                <span>🧪</span>
          ...
          <div className="container mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-4">
            ...
            <div className="mb-3 p-3 bg-gray-800 rounded-lg border border-gray-700">
              <div className="flex items-center gap-2">
                <span
                  className={`text-2xl ${
                    isAutoTestActive ? "animate-pulse" : ""
                  }`}
                >
                  🤖
                </span>
                <div>
                  <p className="text-sm font-semibold text-white">
                    {isAutoTestActive ? "Auto-Test Ativo" : "Auto-Test Pausado"}
                  </p>
                  <p className="text-xs text-gray-400">
                    {isAutoTestActive
                      ? "IA controlando todos os jogadores. Ações sendo executadas automaticamente."
                      : "Clique em '🤖 Auto-Test' para iniciar"}
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Logs */}
            <div className="bg-gray-950 rounded-lg border border-gray-700 p-3 max-h-96 overflow-y-auto">
              {testLogs.length === 0 ? (
                <p className="text-gray-500 text-sm text-center py-4">
                  Nenhum log ainda. Ative o auto-test para começar!
                </p>
              ) : (
                <div className="space-y-2">
                  {testLogs.map((log, index) => (
                    <div
                      key={index}
                      className="bg-gray-800 rounded p-2 text-xs sm:text-sm border border-gray-700"
                    >
