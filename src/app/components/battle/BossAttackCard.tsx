...
export default function BossAttackCard({ boss, onBossSelect }: BossAttackCardProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-red-800 to-red-900 rounded-lg p-3 sm:p-4 border-2 border-red-600">
      ...
      <div className="space-y-2 sm:space-y-3">
        ...
        <div>
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-red-300 text-xs sm:text-sm font-bold">
              {boss.currentHp.toLocaleString()} / {boss.calculatedDp.toLocaleString()}
            </span>
          </div>
        ...
        {/* DP */}
        <div className="bg-red-800 rounded p-2 sm:p-3 border border-red-600">
          <div className="text-[10px] sm:text-xs text-red-300 mb-0.5 sm:mb-1">DP</div>
          <div className="text-lg sm:text-2xl font-bold text-red-200">
        ...
        <button
          ...
          className="w-full px-3 sm:px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-1 sm:gap-2"
        >
          <span className="text-lg sm:text-2xl">⚔️</span>
          <span className="text-xs sm:text-sm">Atacar Boss</span>
        </button>
      </div>
    </div>
  );
